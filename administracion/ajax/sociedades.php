...
if($accion == "grabarSociedad")
{
	...
	if($RegistroEncontrado>0)
	{
		...
		$sql = "UPDATE tolerancia SET tolerancia=$Sociedad_tolerancia WHERE bukrs='$Sociedad_id_sociedad';";
		$result = sqlsrv_query($conexion, $sql);
		$afectadas+=sqlsrv_rows_affected($result);
		sqlsrv_close($conexion);
	...
	{
		$sql = "INSERT INTO [tolerancia]([bukrs],[tolerancia]) VALUES ('$Sociedad_id_sociedad',$Sociedad_tolerancia);";
		$result = sqlsrv_query($conexion, $sql);
		$afectadas=sqlsrv_rows_affected($result);
