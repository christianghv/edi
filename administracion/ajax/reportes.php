...
if($accion == "getDatos")
{
	...
	if($tipo!='0')
	{
		$sql = "
			...
			WHERE ca.fecha BETWEEN CONVERT(DATETIME, '".$fecha." 00:00:00', 103) AND CONVERT(DATETIME, '".$fecha." 23:59:59', 103)
			AND ca.id_tipo = '".$tipo."' 
			AND ca.id_formato <> '2' 
...
if($accion == "ObtenerDatosLog")
{
	...
	$result = sqlsrv_query($link, $SqlProvedores);
	...
	while( $row = sqlsrv_fetch_array($result) )
	{
		...
		$resultIncorrectas = sqlsrv_query($link, $SqlIncorrectas);
...
if($accion == "getDatosDetalle")
{
	...
	$sql = "SELECT status as estatus, descripcion, NumeroFactura as nro_factura, CONVERT(char(12),fecha,114) as hora 
			FROM log_detalle 
