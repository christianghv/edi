	...
	if(chekeados>0)
	{
		...
		$.ajax({
				...
				beforeSend: function () {
				 ...
				 success:  function (response) {
					//alert(response);
					if(response=="	 ERROR: proveedor no unico")
					{
					...
					if(response=="	 ERROR: transporte no registrado")
					{
					...
					if(response=="	 ERROR: transporte no unico")
					{
					...
					$.each(json, function(i, d) {
						...
						//Validar si viene no ok
						if (AgrupacionSugerida[Etiqueta+'_'+d.idenBulto] != null)
						{
							tbody+='<td style="width:8%" data-order="'+AgrupacionSugerida[Etiqueta+'_'+d.idenBulto]+'"><input type="text" style="width:50%;text-align: center;text-transform: uppercase" name="Agrup" id="'+d.InvoiceNumber+'_'+d.PONumber+'" idbulto="'+d.idenBulto+'" value="'+AgrupacionSugerida[Etiqueta+'_'+d.idenBulto]+'" required="required"/></td>';
						}
						else
						{
							//Si no esta registrado la urgencia
							if (AgrupacionSugerida[Etiqueta+'_'+d.PONumber.substring(10,12)] != null)
							{
								tbody+='<td style="width:8%" data-order="'+AgrupacionSugerida[Etiqueta+'_'+d.PONumber.substring(10, 12)]+'"><input type="text" style="width:50%;text-align: center;text-transform: uppercase" name="Agrup" id="'+d.InvoiceNumber+'_'+d.PONumber+'" idbulto="'+d.idenBulto+'" value="'+AgrupacionSugerida[Etiqueta+'_'+d.PONumber.substring(10, 12)]+'" required="required"/></td>';
							}
							else
							{
								//Viene OK
								tbody+='<td style="width:8%" data-order="'+AgrupacionSugerida[Etiqueta+'_ok']+'"><input type="text" style="width:50%;text-align: center;text-transform: uppercase" name="Agrup" id="'+d.InvoiceNumber+'_'+d.PONumber+'" idbulto="'+d.idenBulto+'" value="'+AgrupacionSugerida[Etiqueta+'_ok']+'" required="required"/></td>';
							}
						}
	...
	if(chekeados>0)
	{
		...
		$.ajax({
				...
				beforeSend: function () {
				 ...
				 success:  function (response) {
                    if (response.substring(2,8) == "ERROR:") {
						//alert("1");
                        CerrarThickBox();
					    //alert(response);
                        return false;
  ...
  {	  
		...
		$('[name=gee_chk]').each(function(){
			if($(this).is(':checked')){
				...
				{
					Etiqueta=$(this).parent('td').parent('tr').find('td:eq(11) input').val();
				}
				var item = {
										...
										if(d.TYPE=="S" && d.LOG_NO=="EE")
										{
											$('[name=gee_chk]').each(function(){
												...
												{	
													...
													{
														Agrupacion=$(this).parent('td').parent('tr').find('td:eq(11) input').val();
													}
		...
		{
			alert("Para generar EE tiene que seleccionar al menos un registros");
		}
  }
