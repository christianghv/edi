...
error_reporting(0);
	...
	require_once("../..//EDI/ajax/SegAjax.php");
	require_once("../../conect/conect.php");
	require_once("../../EDI/funciones/Funciones_carga.php");
	session_start();
	...
	//die();
	//Variables recibidas
	$CantidadBulto=$_REQUEST["CantidadBulto"];
	$CartaPorte=$_REQUEST["CartaPorte"];
	$ETA=$_REQUEST["ETA"];
	$Ruta=$_REQUEST["Ruta"];
		...
		foreach($CrearEE->ArrayDeGEE as $EE)
	{
		...
		$sqlDato="SELECT DISTINCT 
				...
				CAST((CASE detalle.InvoicePosition WHEN '' THEN 0 ELSE ISNULL(detalle.InvoicePosition,0) END) AS DECIMAL(10, 0)) as InvoicePosition,
				detalle.PONumber,
				detalle.POPosition,
				detalle.ProductID,
				...
				AND CAST((CASE detalle856.it_poPosition WHEN '' THEN 0 ELSE ISNULL(detalle856.it_poPosition,0) END) AS DECIMAL(10, 0))= CAST((CASE detalle.POPosition WHEN '' THEN 0 ELSE ISNULL(detalle.POPosition,0) END) AS DECIMAL(10, 0)) 
				AND CAST((CASE detalle856.invoicePosition WHEN '' THEN 0 ELSE ISNULL(detalle856.invoicePosition,0) END) AS DECIMAL(10, 0)) = CAST((CASE detalle.InvoicePosition WHEN '' THEN 0 ELSE ISNULL(detalle.InvoicePosition,0) END) AS DECIMAL(10, 0)) 
				AND detalle856.it_packingsleep='".$EE->Bulto."'
				WHERE detalle.InvoiceNumber='".$EE->InvoiceNumber."' AND detalle.PONumber='".$EE->PONumber."'
				ORDER BY 2;";
		
		
		$file = fopen("wsCrearEE_QueryEnvio.txt","a+");
		fwrite($file,"\n\r ".$sqlDato);
		fclose($file);
		...
		$resultadoSQL = sqlsrv_query($conexion, $sqlDato);	
		$BdArray = array();
		...
		{
			//Llave para posiciones en factura
			$EtiquetaPosicion=$rowBD[InvoiceNumber].'_'.$rowBD[PONumber].'_'.intval($rowBD[POPosition]);
			
			//Verificar si no existe lo agregara al arreglo
			if(!isset($ArrayPosicionesFacturas[$EtiquetaPosicion]))
			{
				$ArrayPosicionesFacturas[$EtiquetaPosicion]=array();
				array_push($ArrayPosicionesFacturas[$EtiquetaPosicion],$rowBD[InvoicePosition]);
			}
			else
			{
				array_push($ArrayPosicionesFacturas[$EtiquetaPosicion],$rowBD[InvoicePosition]);
    ...
    //Se cierra el XML
		...
		/*$file = fopen("wsCrearEE_XMLENVIO.txt","a+");
		fwrite($file,"\n\r ".$XMLstring);
		fclose($file);*/

		//print_r($ArrayPosicionesFacturas);die();
		...
		else
		{
		  ...
		  //echo '<h2>Result</h2><pre>';
			/**
			$file = fopen("wsCrearEE_RESULTADO_SIN_ENCODE.txt","a+");
			fwrite($file,"\n\r ".$resultWSResp);
			fclose($file);
			*/
			...
			if($resultadoRetorno>0)
			{
				...
				foreach($result as $Resp)
				{
					if((string)$Resp[CAMPO1]=="S")
					{
						...
						$numeroActualFactura=$InvoiceNumberRecibido;
						
						//Actualizando por InvoicePosition
						$EtiquetaPosWS=$InvoiceNumberRecibido.'_'.$PONumberRecibido.'_'.intval($POPositionRecibido);
						/**
						$file = fopen("wsCrearEE_Etiqueta.txt","a+");
						fwrite($file,"\n\r ".$EtiquetaPosWS);
						fclose($file);
						*/
						if(isset($ArrayPosicionesFacturas[$EtiquetaPosWS]))
						{
							foreach($ArrayPosicionesFacturas[$EtiquetaPosWS] as $Pos)
							{	
								$sqlUpdateDetail="UPDATE [InvoiceDetail] SET [EntregaEntrante]='".intval($EERecibido)."' 
										WHERE [InvoiceNumber]='".$numeroActualFactura."'
										AND ([EntregaEntrante]='0'
										OR [EntregaEntrante] is null
										OR rtrim(ltrim([EntregaEntrante])) = '')
										AND PONumber='".$PONumberRecibido."'
										AND CAST((CASE POPosition WHEN '' THEN 0 WHEN NULL THEN 0 ELSE POPosition END) AS DECIMAL(10, 0))  =  CAST((CASE '".$POPositionRecibido."' WHEN '' THEN 0 WHEN NULL THEN 0 ELSE '".$POPositionRecibido."' END) AS DECIMAL(10, 0))
										AND CAST((CASE InvoicePosition WHEN '' THEN 0 WHEN NULL THEN 0 ELSE InvoicePosition END) AS DECIMAL(10, 0))  =  CAST((CASE '".$Pos."' WHEN '' THEN 0 WHEN NULL THEN 0 ELSE '".$Pos."' END) AS DECIMAL(10, 0));";
								/**
								$file = fopen("wsCrearEE_RESULTADO_querys.txt","a+");
								fwrite($file,"\n\r ".$sqlUpdateDetail);
								fclose($file);
								*/
		...
		foreach($ArrayFacturasProcesadas as $Factura)
		{
			...
			$sqlUpdateHeader="EXEC [SP_EE_FACTURA] '".$Factura."'";
