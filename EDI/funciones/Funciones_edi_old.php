<?php 
	...
	function getDatosFaltantes_EDI856($Sociedad,$InvoiceNumber,$POPosition,$ProductID)
	{
		...
		$query 	= " SELECT TOP 01 Cabecera.[InvoiceNumber],
					...
					WHERE Cabecera.[InvoiceNumber]='$InvoiceNumber'
					AND Cabecera.Sociedad='$Sociedad'
					AND CAST((CASE DetFatura.[POPosition] WHEN '' THEN 0 WHEN NULL THEN 0 ELSE DetFatura.[POPosition] END) AS DECIMAL(10, 0)) = CAST((CASE '$POPosition' WHEN '' THEN 0 WHEN NULL THEN 0 ELSE '$POPosition' END) AS DECIMAL(10, 0)) 
					AND DetFatura.[ProductID]='$ProductID';";
		//echo $query;
	...
	{
		$borrarQuery.=" delete from [InvoiceDetail] where [InvoiceNumber]='".ms_escape_string($InvoiceDetail['InvoiceNumber'])."' 
						AND InvoicePosition='".ms_escape_string($InvoiceDetail['InvoicePosition'])."';";
	...
	function IngresarPO_HEADER_EDI855_Manual($PO_HEADER)
	{
		...
		//Borrar query
		$query="insert into [PO_HEADER] ([PO_Number],[PO_Date],[ACK_Date],[PO_Items],[PO_ShipTo],[PO_Sociedad]) 
		values 
	...
	{	
		$borrarQuery="DELETE FROM [PO_DETAIL]
					WHERE [PO_Number]='".ms_escape_string($PO_DETAIL['PO_Number'])."' 
					AND [PO_Item]='".ms_escape_string($PO_DETAIL['PO_Item'])."'";
	...
	{
		$borrarQuery=" delete from [856DETAIL] where [segm_id]='".ms_escape_string($EDI856_856DETAIL['segm_Id'])."' 
						AND [it_packingsleep]='".ms_escape_string($EDI856_856DETAIL['it_packingsleep'])."';";
	...
	function Ingresar856BULTO_Manual($EDI856_856BULTO)
	{
		...
		//Borrar query
		$query="Insert into [856BULTO]([idenBulto],[peso],[unidPeso],[volumen],[unidVolumen],[longitud],[ancho],[alto],[unidDimension],[fechaDespacho],[invNumber]) 
	...
	{
		//Borrar query
		$query="EXEC [SP_EE_FACTURA] '$InvoiceNumber';";
