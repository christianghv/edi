...
set_time_limit(600);
error_reporting(0);
    echo "<br>******Inicio ejecución: ". date("H:i:s")." ******<br>";
...
$sql = "SELECT distinct 
		h1.nombreSolicitante, 
		h1.empresaSolicitante, 
		h1.folio,
		convert(char(10),h1.fecha,104) as fecha_creacion,
		usuarioSolicitante,
		convert(varchar(250),objetivoGasto) as objetivo,
		gastos,
		(SELECT     TOP 1 enviadoPor
FROM         dbo.HistorialAprobacionesSFRG
WHERE     (folio = h1.folio)
ORDER BY fechaEnvio DESC) as enviadoPor,
(SELECT     TOP 1 nombreEnviadoPor
FROM         dbo.HistorialAprobacionesSFRG
WHERE     (folio = h1.folio)
ORDER BY fechaEnvio DESC) as nombreEnviadoPor,
(SELECT     TOP 1 enviadoA
FROM         dbo.HistorialAprobacionesSFRG
WHERE     (folio = h1.folio)
ORDER BY fechaEnvio DESC) as enviadoA,
(SELECT     TOP 1 estado
FROM         dbo.HistorialAprobacionesSFRG
WHERE     (folio = h1.folio)
ORDER BY fechaEnvio DESC) as estado,
(SELECT     TOP 1 convert(char(10),fechaEnvio,103) as fechaEnvio
FROM         dbo.HistorialAprobacionesSFRG
WHERE     (folio = h1.folio)
ORDER BY fechaEnvio DESC) as fechaEnvio,
(SELECT TOP 1  enviadoPor FROM         HistorialAprobacionesSFRG
WHERE     (folio =  h1.folio) AND (tipo = 'RENDICION')
ORDER BY fechaEnvio DESC) as Ucajero,
		codCentroCosto, 
		centroCosto, 
		nombreSolicitante, 
		rutSolicitante, 
		codEmpresaSolicitante, 
		empresaSolicitante, 
		convert(varchar(250),objetivoGasto), 
		montoSaldo as saldo_x_devolver, 
		montoSaldo as saldo_x_cobrar, 
        gastos as total_rendicion, 
		h1.folio, 
		'' as persona_autoriza, 
		convert(char(10),h1.fecha,103) as fecha_reg,
		moneda,
		visa,
		amex as orden_servicio,
		archivoAdjunto as url,
		oficinaVentas as sucursal,
		'' as montoDolar,
		'' as montoPesos,
		vales as folio1,
		ufp as fondoPermanente,
		visa,
(SELECT convert(char(10),max(fechaEnvio),103) FROM         HistorialAprobacionesSFRG
WHERE     (folio =  h1.folio) AND (tipo = 'RENDICION') and estado NOT LIKE '%pagado%') as fechaUltEnvio,
	(SELECT docConta FROM DatosContaRG WHERE folio = h1.folio) as docConta,
	h1.textoSaldo, 
	Resumen.posicion,CONVERT(char(10), Resumen.fecha, 103) AS fecha, Resumen.tipoDocumento, Resumen.subTipoFactura, Resumen.numeroFactura,
	Resumen.posDet, Resumen.montoRendido, Resumen.naturaleza, 
	ListadoDisResRG.imputacionGasto, ListadoDisResRG.nombreDetalleGasto, ListadoDisResRG.idObjetoImputacion, 
	ListadoDisResRG.valorObjetoImputacion, ListadoDisResRG.idImporteAsignado, ListadoDisResRG.cuenta,
	(SELECT viaPago FROM DatosContaRG WHERE folio = h1.folio) as viaPago,
	h1.codEmpresaSolicitante,h1.empresaRendicion 
		FROM dbo.CabeceraRG h1 
		INNER JOIN dbo.ResumenRG Resumen ON 
		Resumen.folio=h1.folio 
		INNER JOIN ListadoDistribucionResumenRG  ListadoDisResRG
		ON ListadoDisResRG.folio=h1.folio 
		AND ListadoDisResRG.numeroFactura= Resumen.numeroFactura 
		AND ListadoDisResRG.tipoFactura=Resumen.tipoDocumento 
		AND ListadoDisResRG.subTipoFactura=Resumen.subTipoFactura 
		AND ListadoDisResRG.rutEmisor=Resumen.posDet 
		WHERE h1.id IN(select max(h2.id) from CabeceraRG h2 where h2.folio = h1.folio) ";
	
	$folio = str_replace(',',"','",$folio);
	if(trim($folio)<>'') $sql .= "and h1.folio IN ('$folio') ";
	if(trim($rut_usuario)<>'') $sql .= "and h1.rutSolicitante = '$rut_usuario' ";
	if(trim($desde)<>'' and trim($hasta)<>'') $sql .= "and (convert(datetime,convert(char(10),h1.fecha,103),103) between convert(datetime,'$desde',103) and convert(datetime,'$hasta',103)) ";
	if(trim($empresa)<>'') $sql .= "and h1.empresaSolicitante = '$empresa' ";
	$usuario = str_replace(',',"','",$usuario);
	if(trim($usuario)<>'') $sql .= "and h1.usuarioSolicitante IN ('$usuario') ";
	$ceco = str_replace(',',"','",$ceco);
	if(trim($ceco)<>'') $sql .= "and h1.codCentroCosto IN ('$ceco') ";		

	if(trim($idEstado)<>''){ $sql .= "and (SELECT     TOP 1 estado
	FROM         dbo.HistorialAprobacionesSFRG
	WHERE     (folio = h1.folio)
	ORDER BY fechaEnvio DESC)";
		if($idEstado=='ENVPAGO') $sql .= " IN('ENVIADO A CAJERO','ENVIADO POR ENCARGADO DE PAGO')";
		elseif($idEstado=='APRPAGO') $sql .= " IN('APROBADO POR ENCARGADO DE PAGO','APROBADO POR CAJERO')";
		else $sql .=" like '$idEstado%' ";
	}
	//$sql .= "ORDER BY h1.folio ASC;";
			...
			WHERE     (folio =  h1.folio) AND (tipo = 'RENDICION') and estado NOT LIKE '%pagado%') as fechaUltEnvio,
				...
				Resumen.posicion,CONVERT(char(10), Resumen.fecha, 103) AS fecha, Resumen.tipoDocumento, Resumen.subTipoFactura, Resumen.numeroFactura,
				Resumen.posDet, Resumen.montoRendido, Resumen.naturaleza, 
				'' as imputacionGasto, '' as nombreDetalleGasto, '' as idObjetoImputacion, 
				'' as valorObjetoImputacion, '' as idImporteAsignado, '' as cuenta,
				(SELECT viaPago FROM DatosContaRG WHERE folio = h1.folio) as viaPago,
				h1.codEmpresaSolicitante,h1.empresaRendicion 
					FROM dbo.CabeceraRG h1 
					INNER JOIN dbo.ResumenRG Resumen ON 
					Resumen.folio=h1.folio 
					WHERE h1.id IN(select max(h2.id) from CabeceraRG h2 where h2.folio = h1.folio) 
					AND Resumen.resumen='Kilometraje' ";
	...
	//echo $NombreFinal;
	echo "<br/><br/>";
	echo '<a href="archivos/'.$NombreFinal.'" target="_blank">'.$NombreFinal.'</a>';
    
    echo "<br>******Termino ejecución: ". date("H:i:s")." ******<br>";
?>