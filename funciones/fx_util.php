<?php
	...
	{	
		//echo $stringDate;
		$unixTimestamp = PHPExcel_Shared_Date::ExcelToPHP($stringDate);
		$stringDate = date("d-m-Y", $unixTimestamp);
		//echo "formato: ".$stringDate = PHPExcel_Style_NumberFormat::toFormattedString($excelDate, 'DD-MM-YYYY');
		return $stringDate;
	}
	function getHoraFormateada($excelDate)
	{
		//$stringDate = $excelDate;//PHPExcel_Style_NumberFormat::toFormattedString($excelDate, 'H:i:s');
		$unixTimestamp = PHPExcel_Shared_Date::ExcelToPHP($stringDate);
		$stringDate = date("H:i:s", $unixTimestamp);
		return $stringDate;
	}
	...
	function getHoraMinutoFecha($excelDate)
	{
		...
		$unixTimestamp = PHPExcel_Shared_Date::ExcelToPHP($stringDate);
		$stringDate = date("H:i", $unixTimestamp);
		return $stringDate;
	}
	...
	{
		$stringDate = "2020";//PHPExcel_Style_NumberFormat::toFormattedString($excelDate, 'YYYY');
		return $stringDate;
	}		
	
function getInvoiceNumber2($Sociedad,$InvoiceNumber,$Anio)
	{
